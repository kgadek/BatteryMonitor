...
showBatStatus ratio = void $ Notify.oneShot "Battery status" ("Procent: "++(ratio2str ratio)++"%") "dialog-question" Nothing
	where ratio2str = show . (*) 100
...
batteryAction (ratio:recent:_) | ratio <= 0.20 && recent > 0.20 = showBatStatus ratio
                               ...
                               | ratio <= 0.10 && recent > 0.10 = showBatStatus ratio
								               ...
								               | otherwise                      = return ()
batteryAction (ratio:htail) = batteryAction (ratio:(ratio+1):htail)
